 ...
 * @returns  number
 */
function calculateResult(M, requirements) {
    // 采用二分查找法,必须先对需求进行排序以供后续安排：
    requirements.sort(function (a, b) { return a - b; });
    // 1. 先定义left，初始值为1，定义right，初始值为需求数总和，定义result保存最终result
    var left = 1;
    var right = requirements.reduce(function (acc, cur) { return acc + cur; }, 0);
    var result = right;
 ...
 * 2. 当月工作量不超过给定人力工作量
 */
function canCompleteWithGivenManpower(M, requirements, manPower) {
