 ...
 * @param ip  传入的ip地址
 */
function ipConvert(ip: string): number | string {
  ...
  if (ip.split(".").length > 1) {
    ...
    let ipList = Number(ip).toString(2).padStart(32, "0");
    // 3.2 将32位的二进制分为4组，每组8位，转换为十进制
    const matched = ipList.match(/\d{1,8}/g) || [];
    return matched.map((ip) => parseInt(ip, 2)).join(".");
  }
}
